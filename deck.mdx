...
- easier to use semantic HTML than try to build in accessibility support
...
<Notes>
  ...
  - div button as an example.
  - so much more work to make a div an accessible button than a button element
...
- over 100 semantic elements, but that isn't always enough. 
- it'd be GREAT if we could have something like this, nav bar that has a dropdown list in it.
...
- our user won't know what this is.
- need a way to assign additional semantic meaning to our grouping of elements, or components so that assistive technologies can work with them.
...
- Lots of attributes to use, sometimes hard to know which to use when.
...
- Let's see what we can do with ARIA by going back to our div button component.
...
- CODE TRANSITION
- add in our button role and set it to "button"
...
- that's when we want to pass in a different accessible label using the aria attributes.
...
```jsx 22 file=./code/aria-0.0.0.js title='add "aria-labelledby" or "aria-label" property'
```
...
- if the accessibleLabel prop exists, then i want to render an aria-label attribute on my button.
...
- it will not announce anything for the button content.
...
- true or false based on the button's inactive state set to the aria-disabled attribute.
...
- There are many different roles states and properties we can use, and the wai-aria documentation helps decipher the combination
- the best aria is no aria at all.  Still use semantic html attributes where appropriate.  
...
- CODE TRANSITION
- We're passing an activate function to our button that we can trigger onClick event...
...
- CODE TRANSITION
- our key press event handler needs to only run our activate function if space or enter was pressed.
...
- example: activate modal by pressing button using the keyboard.
...
- How would you build this in react? render/unmount based on component state.
- be mindful: what happens to focus when component gets rendered?
- what happens to focus when my component unmounts?
- Has to be persistent and predictable throughout component life-cycle, that's something we have to build ourselves.
...
- to recap.
- all of our components needs to support keyboard only navigation.
- make sure non-interactive elements can gain keyboard focus using the tabindex attribute
- then consider how a user would navigate your component using a keyboard.
- what are the interactions you have to include in order for your component to respond to key presses.
- and what happens to your keyboard focus throughout your component life-cycle.
...
- Uncovered the places in the wai-aria documentation that help guide our component development. 
...
- Hope these easy tips will make it easier to better understand all of our users
- know how we can build cool shit that works for them.
- make the web an open and inclusive place for everyone.
...
- find slides on my website, and i'll tweet out the link in a few minutes.
