...
```jsx 16 title='add "tabindex" attribute'
const Button = ({ 
  buttonContent = "So much work...", 
  accessibleLabel, 
  isDisabled = false, 
...
```jsx 21 title="add a keyboard event listener"
const Button = ({ 
  buttonContent = "So much work...", 
  accessibleLabel, 
  isDisabled = false, 
...
```jsx 8:12 title='Listen for "Enter" and "Space"'
const Button = ({ 
  buttonContent = "So much work...", 
  accessibleLabel, 
  isDisabled = false, 
...
- Identify which ARIA attributes we need to add to those HTML elements.
- And manage keyboard navigation.
- And we can uncover these requirements from the official docs once we know where to look.
...
- And most importantly, are we considering our users?
...
- Marcy Sutton wrote an amazing blog post last year that I encourage everyone to read.  She talks about how she led user testing at Gatsby to glimpse how users with disabilities navigate apps that have client-side routing.
...
- Let's not forget who our users are and why we're prioritizing accessibility.
